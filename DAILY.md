...
1. **Read** `README.md` (the module contract and rules), `public/src/schedule.js` (past days) and at least two existing games in `public/src/games/` to see the expected level of polish.
2. **Pick tomorrow's concept.**
   ...
   - If nothing fits, invent something fresh. Look at the last ~7 days in `SCHEDULE` and don't repeat a recent setting or type of challenge. Alternate between dodging (asteroids), choosing (quiz lanes, colour gates, number gates) and staying safe (heatwave shade), and try new twists: momentum or drift, mirrored twins, darkness with lightning flashes, wind, rhythm, gravity flips, puzzles.
...
5. **Schedule it:** add `'<id>'` to `GAMES` and `'<tomorrow YYYY-MM-DD>': '<id>'` to `SCHEDULE` in `public/src/schedule.js`. If tomorrow already has an entry, stop: it's done.
6. **Commit and push** to `main`, with the message `Day #<n>: <emoji> <title>` (n from `dayNumber` in `public/src/engine/day.js`).

## Don'ts

- Don't edit or delete past games or past schedule entries.
